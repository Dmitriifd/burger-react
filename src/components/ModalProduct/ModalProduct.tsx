...
const ModalProduct = ({ calories, description, id, image, ingredients, price, title, weight }: Product) => {
  const { isOpenProductModal } = useSelector((state: RootState) => state.modal);
  const dispatch = useAppDispatch();

  const closeModalHandler = ({ target, currentTarget }: React.MouseEvent) => {
    if (target === currentTarget) {
      dispatch(closeModalProduct());
    }
  };

  return isOpenProductModal ? (
      ...
      <div className="modal__main modal-product">
        <div className="modal-product__container">
          ...
          <div className="modal-product__content">
            <img src={`${API_URI}/${image}`} alt={title} className="modal-product__image" />
          ...
          <div className="modal-product__footer">
            <div className="modal-product__add">
              <button className="modal-product__btn" onClick={() => dispatch(addProduct({ id }))}>
                Добавить
              </button>
